...
function excluirPublicacao($deletar, $conn) {
    try {
        ...
        $stmt = $conn->prepare("SELECT cd_post FROM post WHERE cd_post = :id");
        $stmt->bindParam(':id', $deletar, PDO::PARAM_INT);
        $stmt->execute();
        $resultado = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($resultado) {
            $cd_post = $resultado['cd_post'];
            // Exclua os comentários associados ao post
            $stmt = $conn->prepare("DELETE FROM comentario WHERE id_post = :post_id");
            $stmt->bindParam(':post_id', $cd_post, PDO::PARAM_INT);
            $stmt->execute();

            $caminho_pasta = "upload_post/$cd_post/";
            ...
            if (is_dir($caminho_pasta)) {
                ...
                $stmt = $conn->prepare("DELETE FROM post WHERE cd_post = :id");
                $stmt->bindParam(':id', $deletar, PDO::PARAM_INT);
                $stmt->execute();
