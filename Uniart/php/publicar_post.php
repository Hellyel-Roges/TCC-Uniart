    ...
    if ($_FILES["arquivo"]["error"] === UPLOAD_ERR_OK) {
        ...
        if (!arquivo_permitido($tipo_arquivo)) {
            ...
            if (($tipo_arquivo == "video/mp4" || $tipo_arquivo == "video/webm" || $tipo_arquivo == "video/mkv" || $tipo_arquivo == "video/ogv") && $duration_seconds > $limite_video) {
                ...
                $stmt = $conn->prepare("INSERT INTO post (ds_post, dt_post, ds_arquivo, id_categoria, id_perfil) VALUES (:post, NOW(), :arquivo, :id, :perfil)");
                $stmt->bindParam(':post', $_POST['descricao'], PDO::PARAM_STR);
                $stmt->bindParam(':arquivo', $nome_arquivo);
                $stmt->bindParam(':id', $_POST['categoria'], PDO::PARAM_INT);
                $stmt->bindParam(':perfil', $_SESSION['id'], PDO::PARAM_INT);
                $stmt->execute();
                ...
                if ($stmt->rowCount()) {
                    ...
                    if ((isset($nome_arquivo)) and (!empty($nome_arquivo))) {
                        ...
                        $diretorio = "upload_post/$ultimo_cd/";
